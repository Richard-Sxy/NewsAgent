...
对于新闻运营团队，决定：什么内容，什么时间，什么形式，推给哪些用户，推几次，如何判断这一次推送是否有效。

目标 ｜ 典型诉求 ｜ 可能产生的问题
新闻价值 ｜ 重要公共事件必须及时触达 ｜ 重要但不一定高点击
用户增长 ｜ 提升打开率，DAU，留存 ｜ 容易追求标题刺激
内容消费 ｜ 提升阅读时长、完读率、互动率 ｜ 容易陷入流量导向
商业目标 ｜ 会员转化、广告曝光、活动参与 ｜ 可能损害新闻的独立性

## 0.1 新闻运营主要策略

1.重大事件全量推送
2.兴趣人群定向推送
  tag: 科技用户/财经用户/体育用户/本地用户...
  对于不同用户提高权重，提供发送方式
3.地域与场景推送
  台风只重点通知受影响地区
4.热点与趋势推送
  就是我这个智能体的职责，通过事件窗口观察能力，获取热点系数，但是推送部分还需要进行分数化：
  新闻价值 + 用户相关性 + 地域相关性 + 新鲜度 + 可信度 - 频控惩罚 - 重复内容惩罚 - 打扰成本 - 风险惩罚
5.新闻生命周期推送
  维护了一个event_id，在这个id下面做持续推送
6.编辑精选与公共价值推送
  深度调查/公共策略/民生服务/科普与辟谣
7.用户生命周期推送
  新用户(少量高质量热门内容，用户关注频道，保留一定探索内容)
  活跃用户(tag做兴趣精细推荐)
  沉默用户(每周精选，用户长期关注的主题)
  付费用户(独家报道，深度分析，专属栏目等)
8.实验与灰度推送
  先做小规模测试，两种标题，不同发送时间，不同封面，不同目标人群等消息。

## 1. 项目定位

NewsAgent 是构建一套消费企业已有数据、由数据驱动的新闻分析系统。C 端曝光、点击、阅读、播放和互动数据已经由企业数据库提供，本项目不负责埋点采集、日志传输或建设行为数仓：
