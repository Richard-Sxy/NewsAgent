...
class LocalKnowledgeSearchClient:
    ...
    def __init__(self, repository: TencentNewsCacheRepository) -> None:
...
def render_results(results: list[EnrichedHotNews]) -> str:
    ...
    for item in results:
        ...
        for index, reranked in enumerate(item.related_news, start=1):
            ...
            print(f"     {index}. [{related.news_id or '-'}] {related.title}")
            print(
...
async def run(
    *,
    ...
    if offline:
        ...
        return await service.enrich(ranked, related_limit=3)
