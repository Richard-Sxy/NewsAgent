...
import { newsText2Chunks } from '../../common/string/newsTextSplitter';
...
import { createNewsDatasetChunks } from './newsChunk/service';
...
> => {
  ...
  if (
    chunkSettingMode === ChunkSettingModeEnum.news &&
    llmModel &&
    teamId
  ) {
    return createNewsDatasetChunks({
      text: rawText,
      chunkSize,
      overlapRatio: splitProps.overlapRatio,
      llmModel,
      teamId,
      imageIdList
    });
  ...
  if (chunkSettingMode === ChunkSettingModeEnum.news) {
    const { chunks } = newsText2Chunks({
      text: rawText,
      chunkSize,
      overlapRatio: splitProps.overlapRatio
    });

    return chunks.map((q) => ({ q, a: '', indexes: [], imageIdList }));
  }
