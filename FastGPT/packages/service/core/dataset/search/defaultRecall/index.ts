...
import { analyzeSearchQuery, calculateDynamicEmbeddingWeight } from '../queryAnalysis';
import { buildAdaptiveSearchQueryPlan } from '../queryRewrite';
...
export async function searchDatasetData(
  ...
  const originalQuery = reRankQuery.trim() || textQueries.join('\n').trim();
  const originalAnalysis = analyzeSearchQuery(originalQuery);
  const queryPlan =
    usingQueryRewrite && originalQuery
      ? await buildAdaptiveSearchQueryPlan({
          query: originalQuery,
          analysis: originalAnalysis,
          teamId,
          llmModel: queryRewriteModel || getDefaultLLMModel().model,
          timeout: 5000,
          userKey
        })
      : {
          ...
          keywordQueries: [originalQuery],
          entities: [],
          timeExpressions: [],
          numberExpressions: [],
          intent: 'other' as const
        };
  const queryAnalysis = analyzeSearchQuery(queryPlan.semanticQuery);
  const effectiveEmbeddingWeight =
    searchMode === DatasetSearchModeEnum.mixedRecall
      ? calculateDynamicEmbeddingWeight({ analysis: queryAnalysis, baseWeight: embeddingWeight })
      : embeddingWeight;
  ...
  const {
    ...
    semanticQueries: [
      ...new Set(
        [queryPlan.semanticQuery, ...textQueries].map((item) => item.trim()).filter(Boolean)
      )
    ],
    fullTextQueries: [
      ...new Set(
        [originalQuery, ...queryPlan.keywordQueries].map((item) => item.trim()).filter(Boolean)
      )
    ],
  ...
  const textRecallResults = concatWeightedRecallLists([
    { weight: effectiveEmbeddingWeight, list: textEmbeddingRecallResults },
    { weight: 1 - effectiveEmbeddingWeight, list: textFullTextRecallResults }
  ]);
