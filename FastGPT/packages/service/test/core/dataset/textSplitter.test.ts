...
it('routes news mode through the news splitter', async () => {
...
这是新闻的详细正文。`,
  ...
  expect(result[0].q).toContain('文章标题：新闻标题');
  expect(result.some((item) => item.q.includes('段落主题：事件进展'))).toBe(true);
  expect(result.every((item) => item.a === '')).toBe(true);
  expect(result.every((item) => item.indexes.length === 0)).toBe(true);
});
