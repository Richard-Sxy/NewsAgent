...
export const SearchDatasetTestBodySchema = z
  .object({
    ...
    datasetSearchUsingQueryRewrite: z.boolean().optional().meta({
      description: '是否使用新闻查询结构化改写'
    }),
    datasetSearchQueryRewriteModel: z.string().optional().meta({
      description: '新闻查询结构化改写模型'
    }),
