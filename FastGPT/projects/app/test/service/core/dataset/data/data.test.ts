...
import {
  buildDatasetDataFullTextSource,
  createDatasetData,
  deleteDatasetData,
...
describe('Dataset data service', () => {
  ...
  describe('createDatasetData', () => {
    it('should create data, full-text tokens, indexes and remove dataset image ttl', async () => {
      ...
      expect(dataText?.fullTextToken).toContain('manual');
      expect(dataText?.fullTextToken).toContain('index');
      expect(ttl).toBeNull();
    });
  ...
  describe('updateDatasetDataByIndexes', () => {
    it('should update q/a, replace full indexes, record history and delete stale vectors', async () => {
      ...
      const expectedFullTextToken = await jiebaSplit({
        text: 'new question\nnew answer\nnew custom index'
      });
  ...
  describe('buildDatasetDataFullTextSource', () => {
    it('should include custom indexes and exclude duplicate, default and image indexes', () => {
      expect(
        buildDatasetDataFullTextSource({
          q: '正文',
          a: '回答',
          indexes: [
            { type: DatasetDataIndexTypeEnum.custom, text: 'C罗 person' },
            { type: DatasetDataIndexTypeEnum.custom, text: 'C罗 person' },
            { type: DatasetDataIndexTypeEnum.default, text: '带前缀的正文' },
