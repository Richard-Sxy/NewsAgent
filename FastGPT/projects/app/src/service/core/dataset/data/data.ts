 ...
 * 稀疏检索
 *
 * Q/A 保持原有行为；额外加入 custom index，使新闻切分阶段抽取的人名、机构和事实
 * 同时参与 MongoDB 全文召回。default 与图片索引不重复写入，避免正文膨胀和图片 key
 * 污染关键词搜索。完全相同的文本只保留一次。
 */
export const buildDatasetDataFullTextSource = ({
  q,
  a,
  indexes
}: {
  ...
  Array.from(
    new Set(
      [
        ...
        a,
        ...indexes
          .filter((index) => index.type === DatasetDataIndexTypeEnum.custom)
...
export class DatasetDataOperation {
  ...
  async create({
    ...
    // 单独维护分词后的全文检索内容；新闻实体和事实 custom index 也参与关键词召回。
      ...
      [
        {
          ...
          fullTextToken: await jiebaSplit({
            text: buildDatasetDataFullTextSource({ q: indexQ, a, indexes: newIndexes })
          })
        }
      ],
      { session, ordered: true }
  ...
  async updateByIndexes({
    ...
    await mongoSessionRun(async (session) => {
      ...
      // Q/A 和调用方提交的 custom index 都会影响全文召回。
      await MongoDatasetDataText.updateOne(
        { dataId: mongoData._id },
        {
          fullTextToken: await jiebaSplit({
            text: buildDatasetDataFullTextSource({
              q: mongoData.q,
              a: mongoData.a,
              indexes: newIndexes
            })
  ...
  async updateSystemIndexes({
    ...
    await mongoSessionRun(async (session) => {
      ...
      // 上面的 pipeline 会保留并发写入的外部索引；重新读取最终值，避免全文索引漏掉它们。
      const finalData = await MongoDatasetData.findById(mongoData._id)
        .select('indexes')
        .session(session)
        .lean();
      if (!finalData) return Promise.reject('Data not found after system index update');
