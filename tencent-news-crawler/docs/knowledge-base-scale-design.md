...
> 状态更新：2026-09-18。本文是容量与分层索引的设计推演，不是 NewsAgent 当前部署清单，
> 文中的 8000 篇/天、10 年、成本、硬件和 SLO 都是估算假设，不能作为项目实测结果。
> 当前仓库仍由 FastGPT 承担实验知识库的分片、向量化和检索；超大规模冷热分层、独立
> 向量集群、索引迁移与企业 Redis Functions 尚未落地。视频文本化模块已在
> `writing-agent-service` 实现离线链路，但尚未接入本爬虫每日发现与入库主链路。
...
FastGPT 在当前项目中还承载结构化模型应用调用，但同样不是业务事实源；热点指标、运行
状态、审核和配置版本仍由 `writing-agent-service` 的确定性代码与持久化层负责。
