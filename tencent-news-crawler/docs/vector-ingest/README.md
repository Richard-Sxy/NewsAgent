...
> 状态更新：2026-09-18。本文描述的是当前 FastGPT 实验入库链路，不是独立建设的企业
> 向量平台。图文正文、metadata、SQLite 幂等和训练状态检查已经实现；内容保存与索引仍
> 同步耦合，更正/撤稿/删除语义、索引版本治理和视频每日入库尚未完成。
...
- `writing-agent-service` 已实现视频 ASR、画面概括和统一文本化的离线链路，但尚未与本
  爬虫的每日发现及 FastGPT 入库主链路打通。
